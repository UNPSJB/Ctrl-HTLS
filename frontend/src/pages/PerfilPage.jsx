...
import Header from '@/modules/vendor/layout/Header';
import Footer from '@/modules/vendor/layout/Footer';
import axiosInstance from '@/api/axiosInstance';
import { InnerLoading } from '@/components/ui/InnerLoading';
import { capitalizeWords, capitalizeFirst } from '@/utils/stringUtils';
import { toast } from 'react-hot-toast';
import { 
  User, Mail, Phone, Hash, Calendar, Building2,
...
function PerfilPage() {
  ...
  useEffect(() => {
    // Tomamos el id_usuario del user del contexto (el token lo provee)
  ...
  // Cálculos de métricas asumiendo 2% de comisión
    ...
    return (
      <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8 flex items-center justify-center">
          <p className="text-gray-500">No tienes permisos para visualizar esta página.</p>
        </main>
        <Footer />
      </div>
    );
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        {loading ? (
          ...
          <div className="animate-in fade-in duration-500 space-y-8">
            ...
            {/* SECCIÓN 1: Tarjeta de Datos Personales y Hoteles */}
            {vendedorData && (
            ...
            {/* SECCIÓN 3: Pestañas de Historial */}
                ...
                {activeTab === 'ventas' && (
                  <div className="w-full flex-1">
                    <div className="border-b border-gray-100 px-6 py-3 flex justify-between items-center dark:border-gray-700/50">
                      <p className="text-sm text-gray-600 dark:text-gray-400">Revisá el estado de los alquileres que efectuaste.</p>
                      <div className="flex items-center gap-2">
                        <Filter className="h-4 w-4 text-gray-400 shrink-0" />
                        <select
                          value={filtroVentas}
                          onChange={(e) => setFiltroVentas(e.target.value)}
                          className="rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                        >
                          ...
                          {ventasFiltradas.length > 0 ? (
                            ventasFiltradas.map((v) => (
                              <tr key={v.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td className="px-6 py-3.5 text-right text-gray-600 dark:text-gray-300">
                                  ${Number(v.subtotal).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </td>
                                <td className="px-6 py-3.5 text-right font-medium text-gray-900 dark:text-gray-200">
                                  ${(Number(v.subtotal) * 0.02).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </td>
                            ...
                            <tr>
                              <td colSpan="5" className="px-6 py-12 text-center text-gray-400 italic">
                                No posees ventas {filtroVentas !== 'todos' ? `en estado "${filtroVentas}"` : 'registradas aún'}.
                              </td>
                            </tr>
                          )}
                        </tbody>
                ...
                {activeTab === 'pagos' && (
                  <div className="w-full flex-1">
                    <div className="border-b border-gray-100 px-6 py-3 dark:border-gray-700/50">
                      <p className="text-sm text-gray-600 dark:text-gray-400 text-center sm:text-left">Comprobantes de comisiones emitidas a tu favor por la administración.</p>
                    </div>
                    <div className="overflow-x-auto w-full">
                      <table className="w-full text-left text-sm relative">
                        <thead className="bg-gray-50 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:bg-gray-800/80 dark:text-gray-400">
                          <tr>
                            ...
                            <th className="px-6 py-4 text-center">Estado de Operación</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                          {liquidaciones.length > 0 ? (
                            liquidaciones.map((l) => (
                              <tr key={l.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td className="px-6 py-3.5 text-gray-900 dark:text-white">
                                  {new Date(l.fechaEmision).toLocaleDateString('es-AR', { year: 'numeric', month: 'long', day: 'numeric' })}
                                </td>
                                <td className="px-6 py-3.5 text-right font-bold text-emerald-600 dark:text-emerald-400">
                                  ${Number(l.total).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </td>
                                <td className="px-6 py-3.5 text-center">
                                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/30">
                                    <CheckCircle2 className="h-3.5 w-3.5" /> Pagado y Conciliado
                                  </span>
                                </td>
                              </tr>
                            ))
                            ...
                            <tr>
                              <td colSpan="4" className="px-6 py-12 text-center text-gray-400 italic">
                                Al no presentar historial de comisiones facturadas, la lista se refleja vacía.
                              </td>
                            </tr>
                          )}
                        </tbody>
