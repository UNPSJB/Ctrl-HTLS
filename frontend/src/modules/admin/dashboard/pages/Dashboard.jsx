...
// Los subcomponentes KpiCard (ResumenVentasCard), TopRankingCard y GraficoAnual han sido extraídos a src/components/ui/dashboard/
...
export default function Dashboard() {
  ...
  const [hotelesTemporadaAlta, setHotelesTemporadaAlta] = useState([]);
  const [hotelesTemporadaBaja, setHotelesTemporadaBaja] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Adaptar y guardar datos de hoteles en temporada
      if (results[6].status === 'fulfilled' && results[6].value) {
        const result = results[6].value;
        const dataAlta = (result.temporadaAlta || []).map((item) => ({
          nombre: item.hotelNombre,
          fechaInicio: item.temporada.fechaInicio,
          fechaFin: item.temporada.fechaFin,
        }));
        const dataBaja = (result.temporadaBaja || []).map((item) => ({
          nombre: item.hotelNombre,
          fechaInicio: item.temporada.fechaInicio,
          fechaFin: item.temporada.fechaFin,
        }));
        setHotelesTemporadaAlta(dataAlta);
        setHotelesTemporadaBaja(dataBaja);
      } else {
        toast.error('No se pudieron cargar los hoteles en temporada.');
      }
    ...
    >
      ...
      >
        ...
        <HotelesTemporadaCard
          dataAlta={hotelesTemporadaAlta}
          dataBaja={hotelesTemporadaBaja}
        />
