...
export default function HabitacionesTab({
  ...
  // Carga de habitaciones seg煤n el modo
  useEffect(() => {
    if (!isLocalMode) {
      if (isActive) {
        fetchHabitaciones();
  ...
  const fetchGlobalTypes = async () => {
    try {
      ...
      // Solo notificar si no es 404 (sin tipos configurados es un estado v醠ido)
      if (error.response?.status !== 404) {
        toast.error(
          error.response?.data?.error || 'Error al cargar los tipos de habitaci髇',
          { id: 'fetch-tipos-hab' }
        );
      }
    }
  ...
  const handleSave = async (payload, habitacion) => {
    try {
      ...
      toast.error(
        error.response?.data?.error || 'Error al procesar habitaci贸n'
      );
  ...
  const handleDelete = async (habitacionId, tempId) => {
    if (isLocalMode) {
      ...
      try {
        ...
        toast.success('Estado de habitaci贸n actualizado');
        await fetchHabitaciones();
      } catch (error) {
        toast.error('Error al cambiar el estado de la habitaci贸n');
      } finally {
      ...
      <div className="flex-shrink-0 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div className="space-y-1">
          <h2 className="flex items-center gap-2 text-xl font-bold text-gray-900 dark:text-white">
            ...
            Control de Habitaciones F铆sicas
          </h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Administre las unidades reales del hotel, asignando su ubicaci贸n y tipo.
          </p>
        </div>
