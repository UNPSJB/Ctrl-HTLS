 ...
 * Combina la estética premium interactiva para paneles administrativos con 
 * la retrocompatibilidad necesaria para los modales crudos del módulo vendedor.
 */
const Modal = ({
  isOpen = true, // default true por retrocompatibilidad con vendor que lo montan condicionalmente
  onClose,
  title,
  description,
  children,
  ...
  useEffect(() => {
    const handleEsc = (e) => {
      if (e.key === 'Escape' && isOpen && !loading && onClose) onClose();
    };
  ...
  const isLegacyMode = !title;
  const shouldShowFooter = !hideFooter && (!isLegacyMode || footer !== undefined);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      >
        {/* Absolute 'X' for Legacy Mode */}
        {isLegacyMode && showX && onClose && (
          <button
            onClick={onClose}
            disabled={loading}
            className="absolute right-4 top-4 z-20 p-2 rounded-full bg-white/70 hover:bg-white dark:bg-gray-800/80 dark:hover:bg-gray-700 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors shadow-sm disabled:opacity-50"
          >
            <X className="w-5 h-5" />
          </button>
        )}
